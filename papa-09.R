...
bottles <- bottles %>%
  mutate(CTD_salinity_avg = mean(c(CTD_salinity_1, CTD_salinity_2))) %>%
  mutate(CTD_salinity_avg = round(CTD_salinity_avg, 4)) %>%
  filter(PH_bottle != "NA" | DICTA_bottle != "NA")

headers <- c("Cruise_ID", "station", "niskin", "Date", "PH_bottle", "DICTA_bottle", "CTD_salinity_1", "CTD_salinity_2", "CTD_salinity_avg")
...
headers <- c("Cruise_ID", "Date", "DICTA_bottle", "PH_bottle", "CTD_salinity_1")
bottles <- bottles[, headers]
